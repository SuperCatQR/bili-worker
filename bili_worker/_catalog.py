...
``aiohttp`` for the in-callable subtitle fetch, §10). The main ``bili_unit``
...
from bili_worker.protocol import ProtocolError
...
# Map the wire ``audio_quality`` (int, mirrors ``bilibili_api.video.AudioQuality``
# values) or a legacy ``quality`` string to an ``AudioQuality`` enum member.
_QUALITY_STR_MAP: dict[str, Any] = {
    ...
    "HI_RES": getattr(AudioQuality, "HI_RES", AudioQuality._132K),
}


def _resolve_quality(quality: int | str | None) -> Any:
    """Resolve a wire quality specifier to an ``AudioQuality`` enum member.

    The main process (CHO-101) sends ``audio_quality`` as an int that mirrors
    ``AudioQuality``'s value; contract §6.4 also allows a ``quality`` string.
    ``None`` defaults to the lowest (``_64K``).
    """
    if quality is None:
        return AudioQuality._64K
    if isinstance(quality, int):
        try:
            return AudioQuality(quality)
        except ValueError:
            logger.warning("unknown audio_quality int %r; defaulting to 64K", quality)
            return AudioQuality._64K
    key = str(quality).upper().replace(" ", "")
    return _QUALITY_STR_MAP.get(key, AudioQuality._64K)
...
# ---------------------------------------------------------------------------
# HTTP backend init (contract §5 init_http_backend, §13 Q4)
# ---------------------------------------------------------------------------


def _init_http_backend(backend: str = "aiohttp", impersonate: str = "chrome131") -> None:
    """Configure the SDK global HTTP backend (worker side, §5).

    ``curl_cffi`` is an optional anti-detect extra; when requested but absent
    we fall back to ``aiohttp`` (logged). ``aiohttp`` is the always-available
    default.
    """
    try:
...
# ---------------------------------------------------------------------------
# Catalog — all 63 endpoints (33 uid-level + 30 item-level, contract §6.2)
# ---------------------------------------------------------------------------
...
async def _call_page(
    ...
    if entry.kind != "uid":
        raise ProtocolError(f"endpoint {endpoint!r} is kind={entry.kind}, not uid")
...
async def _call_item(
    ...
    ``parent_uid`` is forwarded as ``_uid`` **only** for ``needs_parent_uid``
    endpoints (channel_videos_season|series, upower_qa_detail) — mirroring the
    main-process line order (CHO-101 review S2): the main side nests
    ``parent_uid`` inside ``params.extra`` and only when ``needs_parent_uid``;
    the worker unpacks it there (see ``handle_fetch_item`` in ``__main__``).
    """
    entry = CATALOG_BY_NAME.get(endpoint)
    if entry is None:
        raise ProtocolError(f"unknown endpoint: {endpoint!r}")
    if entry.kind != "item":
        raise ProtocolError(f"endpoint {endpoint!r} is kind={entry.kind}, not item")
