...
Stage 2 Step 3+: full op dispatch with catalog, credential pool, auth, and audio URL.
"""
...
from bili_worker.credential import CredentialPool
...
async def handle_handshake(_params: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "worker": f"bili_worker/{__version__}",
...
async def handle_init_http_backend(params: dict[str, Any]) -> dict[str, Any]:
    """Configure the HTTP backend (contract §6.3)."""
...
async def handle_credential_open(params: dict[str, Any], pool: CredentialPool) -> dict[str, Any]:
    ...
    reload_env = params.get("reload_env", False)
    try:
        ref, has_sessdata = await pool.open(reload_env=reload_env)
        return {"credential_ref": ref, "has_sessdata": has_sessdata}
    except Exception as exc:
        raise credential_error_pack(str(exc)) from exc


async def handle_credential_status(params: dict[str, Any], pool: CredentialPool) -> dict[str, Any]:
    """Check if a credential_ref is valid (contract §6.5)."""
    ref = params["credential_ref"]
...
async def handle_fetch_page(params: dict[str, Any], pool: CredentialPool) -> dict[str, Any]:
    """Execute a uid-level endpoint callable (contract §6.3)."""
    uid = params["uid"]
...
async def handle_fetch_item(params: dict[str, Any], pool: CredentialPool) -> dict[str, Any]:
    """Execute an item-level endpoint callable (contract §6.3)."""
    endpoint = params["endpoint"]
...
async def handle_resolve_audio_url(params: dict[str, Any], pool: CredentialPool) -> dict[str, Any]:
    """Resolve CDN audio URL (contract §6.4)."""
    bvid = params["bvid"]
    page_index = params.get("page_index", 0)
    quality = params.get("quality", "64K")
    cred_ref = params.get("credential_ref")
    credential = await pool.resolve(cred_ref)
...
async def handle_login_save_env(params: dict[str, Any], pool: CredentialPool) -> dict[str, Any]:
    """Save credential to .env (contract §6.6)."""
    from ._auth import login_save_env
    cred_ref = params["credential_ref"]
    env_path = params.get("env_path", ".env")
    cred = await pool.resolve(cred_ref)
    if cred is None:
        raise ValueError(f"credential_ref {cred_ref!r} not found")
...
class _Shutdown(Exception):
    """Internal signal to stop the dispatch loop."""
...
async def dispatch(req: Request, pool: CredentialPool) -> dict[str, Any]:
    ...
    try:
        if op == "handshake":
            ...
            resp = ok_response(req_id, data)
            raise _Shutdown(resp)
        else:
            data = await handle_unknown_op(op, params)

        return ok_response(req_id, data)

    except _Shutdown as sd:
        # The exception carries the response frame.
        return sd.args[0]
...
async def run_loop() -> None:
    ...
    Uses executor-based stdin reading (``sys.stdin.readline`` in a thread)
